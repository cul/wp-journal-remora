...
function cfct_options_input($args) {
	...
	switch ($type) {
		...
		case 'hidden':
			...
			break;
		case 'media':
            // Get the preview, if possible
            $preview = wp_get_attachment_image_src(get_attachment_id_from_src($value), array(50, 50));
            
            if ($preview[0])
                $previewElement = 'Preview:</small><br/><a href="' . $value . '"><img class="img-polaroid" src="' . $preview[0] . '" width="' . $preview[1] . '" height="' . $preview[2] . '" /></br><small>Click to view full size</a>';
            elseif ($value)
                $previewElement = 'Preview:<br/><div class="alert alert-info">Thumbnail could not be generated, but probably not a problem.<br/><a href="' . $value . '"><strong>Click to view image</strong></a></div>';

            // Construct the HTML
            $class = $class . ' media_button';

            $html .= '<input id="' . esc_attr($name) . '" name="' . esc_attr($name) . '" type="text" value="' . $value . '" class="' . $class . '" /><input id="' . esc_attr($name) . '_button" class="media_button" type="button" value="Select Image" />' . '<div class="preview-img">' . $previewElement . '</div>';
            break;
		default:
